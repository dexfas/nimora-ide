...
// SHUNCODE_FORK_BEGIN: bundled language pack bootstrap (first-launch localization)
import { syncBuiltInLanguagePacks } from './shunCodeLanguagePackBootstrap.js';
// SHUNCODE_FORK_END
...
export async function resolveNLSConfiguration({ userLocale, osLocale, userDataPath, commit, nlsMetadataPath }: IResolveNLSConfigurationContext): Promise<INLSConfiguration> {
	...
	// SHUNCODE_FORK_BEGIN: make bundled language packs resolvable on first launch
	// (skipped for the `en`/pseudo/dev early return above, matching stock behavior).
	try {
		await syncBuiltInLanguagePacks(userDataPath, nlsMetadataPath);
	} catch (error) {
		console.error('Bundled language pack sync failed.', error);
	}
	// SHUNCODE_FORK_END
