	...
	function convertToolSpecificData(data: any): any {
		...
		if ('command' in data && 'language' in data) {
			...
			return {
				...
				presentationStyle: data.presentationStyle === 'shuncode' ? 'shuncode' : undefined,
				presentationKind: ['files', 'search', 'edit', 'terminal', 'diagnostics', 'lsp', 'generic'].includes(data.presentationKind) ? data.presentationKind : undefined,
				...
				diffPreview: Array.isArray(data.diffPreview) ? data.diffPreview.map((file: any) => ({
					path: typeof file?.path === 'string' ? file.path : '',
					truncated: typeof file?.truncated === 'boolean' ? file.truncated : undefined,
					hunks: Array.isArray(file?.hunks) ? file.hunks.map((hunk: any) => ({
						truncated: typeof hunk?.truncated === 'boolean' ? hunk.truncated : undefined,
						lines: Array.isArray(hunk?.lines) ? hunk.lines.map((line: any) => ({
							kind: ['context', 'add', 'delete'].includes(line?.kind) ? line.kind : 'context',
							oldLine: typeof line?.oldLine === 'number' ? line.oldLine : undefined,
							newLine: typeof line?.newLine === 'number' ? line.newLine : undefined,
							text: typeof line?.text === 'string' ? line.text : '',
						})) : [],
					})) : [],
				})).filter((file: any) => file.path) : undefined,
				summary: typeof data.summary === 'string' ? data.summary : undefined,
				detailsLabel: typeof data.detailsLabel === 'string' ? data.detailsLabel : undefined,
				items: Array.isArray(data.items) ? data.items.map((item: any) => ({
					label: typeof item?.label === 'string' ? item.label : '',
					description: typeof item?.description === 'string' ? item.description : undefined,
					resource: item?.resource instanceof types.Location
						? Location.from(item.resource)
						: item?.resource ? URI.revive(item.resource) : undefined,
					added: typeof item?.added === 'number' ? item.added : undefined,
					removed: typeof item?.removed === 'number' ? item.removed : undefined,
				})).filter((item: any) => item.label) : undefined,
				metrics: Array.isArray(data.metrics) ? data.metrics.map((metric: any) => ({
					label: typeof metric?.label === 'string' ? metric.label : '',
					value: typeof metric?.value === 'string' ? metric.value : '',
				})).filter((metric: any) => metric.label && metric.value) : undefined,
			};
...
export namespace ChatAgentRequest {
	export function to(request: IChatAgentRequest, location2: vscode.ChatRequestEditorData | vscode.ChatRequestNotebookData | undefined, model: vscode.LanguageModelChat, modelConfiguration: IStringDictionary<unknown> | undefined, diagnostics: readonly [vscode.Uri, readonly vscode.Diagnostic[]][], tools: Map<vscode.LanguageModelToolInformation, boolean>, extension: IRelaxedExtensionDescription, logService: ILogService): vscode.ChatRequest {
		...
		const requestWithAllProps: vscode.ChatRequest = {
			...
			shunCodeBranchIntent: request.shunCodeBranchIntent,
		};
		...
		if (!isProposedApiEnabled(extension, 'chatParticipantPrivate')) {
			...
			// eslint-disable-next-line local/code-no-any-casts
			delete (requestWithAllProps as any).shunCodeBranchIntent;
		}
