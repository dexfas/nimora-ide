...
export class McpService extends Disposable implements IMcpService {
	...
	private async _autostart(autoStartConfig: McpAutoStartValue, state: ISettableObservable<IAutostartResult>, token: CancellationToken) {
		...
		// Retry errored servers on every autostart pass so a transient failure
		// (e.g. a remote HTTP MCP endpoint that was briefly unreachable) recovers
		// automatically on the next chat request. Only disabled servers are skipped.
		const candidates = this.servers.get().filter(s =>
			isContributionEnabled(s.enablement.get())
		);

		let todo = new Set<IMcpServer>();
		// A server that reached an error state must be retried even when its tools
		// were already cached (cacheState Live): otherwise a transient failure only
		// recovers after a manual Restart.
		const inError = (s: IMcpServer) => s.connectionState.get().state === McpConnectionState.Kind.Error;
		if (autoStartConfig === McpAutoStartValue.OnlyNew) {
			todo = new Set(candidates.filter(s => s.cacheState.get() === McpServerCacheState.Unknown || inError(s)));
		} else if (autoStartConfig === McpAutoStartValue.NewAndOutdated) {
			todo = new Set(candidates.filter(s => {
				const c = s.cacheState.get();
				return c === McpServerCacheState.Unknown || c === McpServerCacheState.Outdated || inError(s);
			}));
		}
