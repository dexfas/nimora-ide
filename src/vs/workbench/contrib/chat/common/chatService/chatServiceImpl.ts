...
export class ChatService extends Disposable implements IChatService {
	...
	async resendRequest(request: IChatRequestModel, options?: IChatSendRequestOptions): Promise<void> {
		...
		const resendOptions: IChatSendRequestOptions = {
			...options,
			// A retried branch/merge answer must keep its round intent, otherwise
			// the extension treats it as a brand new question and opens a new
			// branch group (the question appears twice instead of one question
			// with several answers in a mergeable container).
			shunCodeBranchIntent: options?.shunCodeBranchIntent ?? request.shunCodeBranchIntent,
	...
	async sendRequest(sessionResource: URI, request: string, options?: IChatSendRequestOptions): Promise<ChatSendResult> {
		...
		// A ShunCode multi-model round is a deliberate empty send: the merge
		// summary action submits an empty message with a 'merge' intent, and the
		// extension builds the merge prompt from the branch history. Do not
		// reject it like an accidental empty submission.
		if (!request.trim() && !hasExplicitFileOrImageAttachment && !options?.slashCommand && !options?.agentId && !options?.agentIdSilent && !options?.shunCodeBranchIntent) {
