...
export interface ILanguageModelsService {
	...
	/**
	 * Returns a provider group's non-secret configuration and which secret fields are configured.
	 * Secret values are intentionally never returned to UI callers.
	 */
	getLanguageModelsProviderConfiguration?(vendorId: string, providerGroupName?: string): Promise<{ values: IStringDictionary<unknown>; configuredSecretKeys: readonly string[] } | undefined>;

	/**
	 * Updates a provider group from an inline settings surface. Omitted secret fields preserve
	 * their previously stored values.
	 */
	setLanguageModelsProviderConfiguration?(vendorId: string, values: IStringDictionary<unknown>, providerGroupName?: string): Promise<void>;
...
export class LanguageModelsService implements ILanguageModelsService {
	...
	async configureLanguageModelsProviderGroup(vendorId: string, providerGroupName?: string): Promise<void> {
		...
		const existing = providerGroupName
			? languageModelProviderGroups.find(g => g.vendor === vendorId && g.name === providerGroupName)
			: languageModelProviderGroups.find(g => g.vendor === vendorId && vendorId === 'shuncode');

		const name = existing?.name
			?? (providerGroupName ?? (vendorId === 'shuncode' ? vendor.displayName : await this.promptForName(languageModelProviderGroups, vendor, existing)));
		if (!name) {
			return;
		}
	...
	async getLanguageModelsProviderConfiguration(vendorId: string, providerGroupName?: string): Promise<{ values: IStringDictionary<unknown>; configuredSecretKeys: readonly string[] } | undefined> {
		const vendor = this.getVendors().find(candidate => candidate.vendor === vendorId);
		const schema = vendor?.configuration as IJSONSchema | undefined;
		if (!vendor || !schema) {
			return undefined;
		}

		const group = this._languageModelsConfigurationService.getLanguageModelsProviderGroups().find(candidate =>
			candidate.vendor === vendorId && (!providerGroupName || candidate.name === providerGroupName));
		if (!group) {
			return undefined;
		}

		const resolved = await this._resolveConfiguration(group, schema);
		const values: IStringDictionary<unknown> = { ...resolved };
		const configuredSecretKeys: string[] = [];
		for (const [key, propertySchema] of Object.entries(schema.properties ?? {})) {
			if (!propertySchema.secret) {
				continue;
			}
			if (isString(resolved[key]) && resolved[key].trim()) {
				configuredSecretKeys.push(key);
			}
			delete values[key];
		}

		return { values, configuredSecretKeys };
	}

	async setLanguageModelsProviderConfiguration(vendorId: string, values: IStringDictionary<unknown>, providerGroupName?: string): Promise<void> {
		const vendor = this.getVendors().find(candidate => candidate.vendor === vendorId);
		const schema = vendor?.configuration as IJSONSchema | undefined;
		if (!vendor || !schema) {
			throw new Error(`Vendor ${vendorId} is not configurable.`);
		}

		const groups = this._languageModelsConfigurationService.getLanguageModelsProviderGroups();
		const existing = providerGroupName
			? groups.find(candidate => candidate.vendor === vendorId && candidate.name === providerGroupName)
			: groups.find(candidate => candidate.vendor === vendorId);
		const existingConfiguration = existing ? await this._resolveConfiguration(existing, schema) : {};
		const suppliedValues: IStringDictionary<unknown> = { ...values };
		for (const [key, propertySchema] of Object.entries(schema.properties ?? {})) {
			if (propertySchema.secret && suppliedValues[key] === '') {
				delete suppliedValues[key];
			}
		}

		const configuration: IStringDictionary<unknown> = { ...existingConfiguration, ...suppliedValues };
		...
		for (const requiredKey of schema.required ?? []) {
			if (!configuration[requiredKey]) {
				throw new Error(localize('models.requiredProviderValue', "Enter {0}.", schema.properties?.[requiredKey]?.title ?? requiredKey));
			}
		}

		const name = existing?.name ?? providerGroupName ?? vendor.displayName;
		const resolvedGroup = {
			...await this._resolveLanguageModelProviderGroup(name, vendorId, configuration, schema),
			settings: existing?.settings
		};
		if (existing) {
			await this._languageModelsConfigurationService.updateLanguageModelsProviderGroup(existing, resolvedGroup);
			await this._deleteSecretsInConfiguration(existing, schema);
		} else {
			await this._languageModelsConfigurationService.addLanguageModelsProviderGroup(resolvedGroup);
		}
	...
	private async promptForConfiguration(groupName: string, configuration: IJSONSchema, existing: IStringDictionary<unknown> | undefined): Promise<IStringDictionary<unknown> | undefined> {
		...
		for (const property of Object.keys(configuration.properties)) {
			...
			// Prompt for required properties, properties already configured for the
			// group, and secret properties when creating a brand-new group (secrets
			// cannot be entered through the JSON file, so the prompt is their only
			// entry point). Optional non-secret properties are configured in the
			// language models JSON file instead of a long prompt chain.
			const isSecret = typeof propertySchema !== 'boolean' && !!propertySchema.secret;
			const shouldPrompt = required
				|| (existing !== undefined && existing[property] !== undefined)
				|| (existing === undefined && isSecret);
			if (!shouldPrompt) {
				continue;
			}
