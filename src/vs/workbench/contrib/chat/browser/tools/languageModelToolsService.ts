...
/**
 * ShunCode IDE terminal tools. Their results are raw terminal transcripts; the Chat UI
 * renders them as compact, collapsible terminal steps instead of unstructured text.
 */
const SHUNCODE_TERMINAL_TOOL_IDS = new Set([
	'shuncode_run_command',
	'shuncode_get_command_output',
	'shuncode_send_command_input',
]);

function isShunCodeTerminalTool(toolId: string): boolean {
	return SHUNCODE_TERMINAL_TOOL_IDS.has(toolId);
}

/** Extracts the captured terminal output between the tool result's OUTPUT markers. */
function extractShunCodeOutputBlock(text: string): string | undefined {
	const match = text.match(/--- OUTPUT BEGIN ---\r?\n([\s\S]*?)\r?\n--- OUTPUT END ---/);
	if (!match) {
		return undefined;
	}
...
/** Parses `exit_code: N` / `exit_code: null` from a ShunCode terminal tool result. */
function parseShunCodeExitCode(text: string): number | null | undefined {
...
export class LanguageModelToolsService extends Disposable implements ILanguageModelToolsService {
	...
	private ensureToolDetails(dto: IToolInvocation, toolResult: IToolResult, toolData: IToolData, toolInvocation: ChatToolInvocation | undefined): void {
		if (!toolResult.toolResultDetails) {
			const terminalDetails = this.getShunCodeTerminalDetails(dto, toolResult);
			if (terminalDetails) {
				toolResult.toolResultDetails = terminalDetails;
				return;
			}
		}
	...
	/**
	 * ShunCode terminal tools (`shuncode_run_command`, `shuncode_get_command_output`,
	 * `shuncode_send_command_input`) return the full terminal transcript as plain text.
	 * Shape it as structured input/output details so the Chat UI renders a compact,
	 * collapsible terminal step: the command as the input block (shell syntax), the captured
	 * output as the output block, and a failure state derived from the exit code. Without
	 * this the raw transcript would otherwise surface as unstructured text and break the
	 * step-list layout.
	 */
	private getShunCodeTerminalDetails(dto: IToolInvocation, toolResult: IToolResult): IToolResultInputOutputDetails | undefined {
		if (!isShunCodeTerminalTool(dto.toolId)) {
			return undefined;
		}
		const text = toolResult.content.map(part => part.kind === 'text' ? part.value : '').join('\n');
		const parameters = dto.parameters as Record<string, unknown>;
		const input = dto.toolId === 'shuncode_run_command'
			? (typeof parameters.command === 'string' ? parameters.command : '')
			: dto.toolId === 'shuncode_send_command_input'
				? (typeof parameters.input === 'string' ? parameters.input : '')
				: (typeof parameters.command_id === 'string' ? parameters.command_id : '');
		const output = extractShunCodeOutputBlock(text) ?? text;
		const exitCode = parseShunCodeExitCode(text);
		return {
			input,
			inputLanguage: 'shell',
			output: [{ type: 'embed', isText: true, value: output }],
			isError: exitCode !== undefined && exitCode !== null && exitCode !== 0,
		};
	}
