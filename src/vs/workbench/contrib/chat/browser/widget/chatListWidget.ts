...
import { IChatPendingDividerViewModel, IChatRequestViewModel, IChatResponseViewModel, IChatViewModel, isRequestVM, isResponseVM } from '../../common/model/chatViewModel.js';
...
export class ChatListWidget extends Disposable {
	...
	constructor(
		...
		@IChatBranchService private readonly chatBranchService: IChatBranchService,
	) {
		super();
		...
		// ShunCode multi-model: re-apply the variant container filter whenever the
		// branch service state changes (variant registration, ◀ ▶ switch, delete).
		this._register(this.chatBranchService.onDidChange(uri => {
			if (this._viewModel && uri.toString() === this._viewModel.sessionResource.toString()) {
				this.refresh();
			}
		}));
	...
	/**
	 * ShunCode multi-model: mirror the extension's branch group metadata into
	 * the branch service so variant switching works even for rows the container
	 * filter hides (registration no longer depends on row rendering).
	 */
	private _syncShunCodeBranchVariants(): void {
		const viewModel = this._viewModel;
		if (!viewModel) {
			return;
		}
		const variants: { groupId: string; requestId: string; responseId: string; kind: 'branch' | 'merge'; modelId?: string; canceled?: boolean }[] = [];
		for (const request of viewModel.model.getRequests()) {
			const response = request.response;
			const metadata = response?.result?.metadata as Record<string, unknown> | undefined;
			const group = metadata?.['shuncodeBranchGroup'] as { groupId?: unknown; kind?: unknown } | undefined;
			if (!group || typeof group.groupId !== 'string' || !group.groupId || !response) {
				continue;
			}
			const branchModel = metadata?.['shuncodeBranchModel'] as { modelId?: unknown } | undefined;
			variants.push({
				groupId: group.groupId,
				requestId: request.id,
				responseId: response.id,
				kind: group.kind === 'merge' ? 'merge' : 'branch',
				modelId: typeof branchModel?.modelId === 'string' ? branchModel.modelId : undefined,
				canceled: metadata?.['canceled'] === true,
			});
		}
		this.chatBranchService.synchronize(viewModel.sessionResource, variants);
	}

	/**
	 * ShunCode multi-model: each branch group renders as a single question row
	 * plus a variant container that shows exactly one answer at a time. Variant
	 * re-ask request rows and non-active responses are hidden here.
	 */
	private _applyShunCodeBranchFilter(items: (IChatRequestViewModel | IChatResponseViewModel | IChatPendingDividerViewModel)[]): (IChatRequestViewModel | IChatResponseViewModel | IChatPendingDividerViewModel)[] {
		const viewModel = this._viewModel;
		if (!viewModel) {
			return items;
		}
		const sessionResource = viewModel.sessionResource;
		const groups = this.chatBranchService.getGroups(sessionResource);
		if (groups.length === 0) {
			return items;
		}
		const requests = viewModel.model.getRequests();

		// A request whose response is still streaming is the in-flight variant;
		// its response takes over the container until the metadata registers it.
		let inFlightRequestId: string | undefined;
		let inFlightGroupId: string | undefined;
		for (const request of requests) {
			if (request.shunCodeBranchIntent && request.response?.isComplete !== true) {
				inFlightRequestId = request.id;
				inFlightGroupId = request.shunCodeBranchIntent.groupId;
			}
		}

		const hiddenRequestIds = new Set<string>();
		const hiddenResponseRequestIds = new Set<string>();
		const activeGroupId = this.chatBranchService.getActiveGroupId(sessionResource);
		for (const group of groups) {
			let activeRequestId = group.activeRequestId;
			if (inFlightRequestId) {
				const registered = group.variants.some(v => v.requestId === inFlightRequestId);
				const belongs = (inFlightGroupId && inFlightGroupId === group.id)
					|| (!inFlightGroupId && !registered && group.id === activeGroupId);
				if (belongs && !registered) {
					activeRequestId = inFlightRequestId;
				}
			}
			for (const variant of group.variants) {
				if (variant.requestId !== group.baseRequestId) {
					hiddenRequestIds.add(variant.requestId);
				}
				if (variant.requestId !== activeRequestId) {
					hiddenResponseRequestIds.add(variant.requestId);
				}
			}
		}

		// Completed re-ask rows (branch/merge turns) repeat the question; hide
		// them unless they are the base question of their own (fresh) group.
		const groupBaseIds = new Set(groups.map(g => g.baseRequestId));
		for (const request of requests) {
			if (request.shunCodeBranchIntent && !groupBaseIds.has(request.id)) {
				hiddenRequestIds.add(request.id);
			}
		}

		return items.filter(item => {
			if (isRequestVM(item)) {
				return !hiddenRequestIds.has(item.id);
			}
			if (isResponseVM(item)) {
				return !hiddenResponseRequestIds.has(item.requestId);
			}
			return true;
		});
	}
	...
	refresh(): void {
		...
		this._syncShunCodeBranchVariants();
		const items = this._applyShunCodeBranchFilter(this._viewModel.getItems());
