...
export class ModelPickerWidget extends Disposable {
	...
	show(anchor?: HTMLElement): void {
		...
		const availableModels = this._delegate.getModels();
		const shunCodeVendors = new Set(['shuncode', 'shuncode-codex']);
		const hasShunCodeProvider = this._languageModelsService.getVendors().some(vendor => shunCodeVendors.has(vendor.vendor));
		// ShunCode exposes explicitly configured API and Codex providers. Their settings pages
		// already own model visibility, so the picker should mirror both lists as-is
		// instead of promoting the current model and wrapping the remainder in
		// provider/"Other Models" sections.
		const models = hasShunCodeProvider
			? availableModels.filter(model => shunCodeVendors.has(model.metadata.vendor))
			: availableModels;
		const delegatePresentation = this._delegate.getPresentationOptions();
		const presentation = hasShunCodeProvider
			? {
