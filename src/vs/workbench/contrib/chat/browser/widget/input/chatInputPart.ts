...
export class ChatInputPart extends Disposable implements IHistoryNavigationWidget {
	...
	private inputHasBranch: IContextKey<boolean>;
	...
	constructor(
		...
		this.inputHasBranch = ChatContextKeys.inputHasBranch.bindTo(contextKeyService);
		this.chatCursorAtTop = ChatContextKeys.inputCursorAtTop.bindTo(contextKeyService);

		// ShunCode multi-model: refresh the branch-send context key whenever the
		// mode, request progress or the request list (previous turn) changes.
	...
	*/
	...
	private _updateInputContentContextKeys(): void {
		...
		this.inputHasBranch.set(!inputHasText && !hasSendableContent && this.isBranchEligible());
	}

	/**
	 * ShunCode multi-model: whether an empty send should re-answer the previous
	 * question as a new branch variant. Requires Plan mode, an existing turn,
	 * no request in progress and the shuncode.multiModel.enabled setting.
	 */
	isBranchEligible(): boolean {
		if (!(this.configurationService.getValue<boolean>('shuncode.multiModel.enabled') ?? true)) {
			return false;
		}
		// 'plan' covers a future built-in plan mode; the ShunCode Plan custom
		// agent reports its agent-file URI as the mode id, so match it by name.
		const mode = this._currentModeObservable.get();
		const isPlanMode = mode.id === 'plan'
			|| mode.name.get().trim().toLowerCase() === 'shuncode plan';
		if (!isPlanMode) {
			return false;
		}
		const model = this._widget?.viewModel?.model;
		...
		return model.getRequests().length > 0;
	}
