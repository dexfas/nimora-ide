...
type ChatSpeedTierChangeClassification = {
	owner: 'lramos15';
	comment: 'Reporting when the model speed tier is changed';
	model: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; comment: 'The model the speed tier was changed for' };
	fromValue: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; comment: 'The previous speed tier value' };
	toValue: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; comment: 'The new speed tier value' };
};

type ChatSpeedTierChangeEvent = {
	model: string | TelemetryTrustedValue<string>;
	fromValue: string;
	toValue: string;
};
...
export class ModelPickerConfiguration {
	...
	renderButton(button: HTMLElement, compact: boolean, noModelsAvailable: boolean): void {
		...
		const speedConfig = this._getConfigProperty('speed');
		const tokensConfig = this._getConfigProperty('tokens');
		if (!model || noModelsAvailable || (!effortConfig && !speedConfig && !tokensConfig)) {
		...
		if (effortConfig) {
			...
			labelParts.push(compact
				? effortLabel
				: localize('chat.modelPicker.effortLabel', "Thinking: {0}", effortLabel));
			ariaParts.push(localize('chat.modelPicker.effortAriaLabel', "Thinking Effort: {0}", effortLabel));
		}
		if (speedConfig) {
			const enumIndex = speedConfig.schema.enum?.indexOf(speedConfig.value) ?? -1;
			const speedLabel = enumIndex >= 0 && speedConfig.schema.enumItemLabels?.[enumIndex]
				? speedConfig.schema.enumItemLabels[enumIndex]
				: String(speedConfig.value);
			labelParts.push(compact
				? speedLabel
				: localize('chat.modelPicker.speedLabel', "Speed: {0}", speedLabel));
			ariaParts.push(localize('chat.modelPicker.speedAriaLabel', "Speed: {0}", speedLabel));
		}
		// Keep the reasoning selector available in narrow/compact chat layouts,
		// similar to ChatGPT's dedicated effort pill. Context size stays hidden in
		// compact mode to avoid making the input toolbar unnecessarily wide.
		if (tokensConfig && !compact) {
	...
	private _buildItems(): IActionListItem<IActionWidgetDropdownAction>[] {
		...
		appendConfigSection(
			'speed',
			localize('chat.speed.header', "Speed"),
			(value, enumLabel) => enumLabel ?? String(value),
			(value, previousValue) => this._telemetryService.publicLog2<ChatSpeedTierChangeEvent, ChatSpeedTierChangeClassification>('chat.speedTierChange', {
				model: model.metadata.vendor === 'copilot' ? new TelemetryTrustedValue(modelIdentifier) : 'unknown',
				fromValue: previousValue,
				toValue: String(value),
			}),
