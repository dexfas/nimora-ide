...
import { IChatSimpleToolInvocationData } from '../../../common/chatService/chatService.js';
...
export class ChatCollapsibleInputOutputContentPart extends Disposable {
	...
	constructor(
		...
		shimmer: boolean,
		private readonly summaryPresentation: IChatSimpleToolInvocationData | undefined,
	...
	private createMessageContents() {
		const presentation = this.summaryPresentation;
		if (!presentation?.items?.length) {
			return this.createTechnicalContents();
		}

		const root = dom.$('.chat-tool-summary-presentation');
		const items = dom.$('.chat-tool-summary-items');
		for (const item of presentation.items) {
				const row = dom.$('.chat-tool-summary-item');
				const label = item.resource ? dom.$('a.chat-tool-summary-item-label') : dom.$('span.chat-tool-summary-item-label');
				label.textContent = item.label;
				if (item.resource) {
					const resource = URI.isUri(item.resource)
						? item.resource
						: isUriComponents(item.resource)
							? URI.revive(item.resource)
							: URI.revive(item.resource.uri);
					(label as HTMLAnchorElement).href = resource.toString();
					this._register(dom.addDisposableListener(label, dom.EventType.CLICK, event => {
				...
				if (item.description) {
					const description = dom.$('span.chat-tool-summary-item-description');
					description.textContent = item.description;
					row.appendChild(description);
				}
				if (item.added !== undefined || item.removed !== undefined) {
					const diff = dom.$('span.chat-tool-summary-item-diff');
					const added = item.added !== undefined ? `+${item.added}` : '';
					const removed = item.removed !== undefined ? `−${item.removed}` : '';
					diff.textContent = [added, removed].filter(Boolean).join(' ');
	...
	private createTechnicalContents() {
