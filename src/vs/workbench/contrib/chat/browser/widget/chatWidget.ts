...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	constructor(
		...
		// ShunCode multi-model: re-render rows when branch state changes (a new
		// variant registers, a variant is adopted, or canonical state hydrates).
		this._register(this.chatBranchService.onDidChange(sessionResource => {
			if (!this.viewModel || this.viewModel.sessionResource.toString() !== sessionResource.toString()) {
				return;
			}
			// Defer so the re-render never happens inside the render cycle that registered the variant.
	...
	async acceptInput(query?: string, options?: IChatAcceptInputOptions): Promise<IChatResponseModel | undefined> {
		...
		const branchQuery = this._computeShunCodeBranchQuery(query, options);
		if (branchQuery === undefined && query === undefined && !options?.shunCodeBranchIntent && !this.getInput().trim() && this.input.isBranchEligible()) {
			// Branch state was active but no anchor question text is available (e.g. the
			// previous turn is a merge row): do not send an empty message.
			return undefined;
		}
		const effectiveOptions: IChatAcceptInputOptions | undefined = branchQuery !== undefined
			? { ...options, shunCodeBranchIntent: { kind: 'branch', groupId: undefined } as const }
			: options;
		return this._acceptInput(query !== undefined ? { query } : branchQuery !== undefined ? { query: branchQuery } : undefined, effectiveOptions);
	}

	/**
	 * ShunCode multi-model: an empty input send in Plan mode re-answers the
	 * previous question as a new branch variant of the same turn. Returns the
	 * anchor question text so the variant row is visible and the round's
	 * question stays discoverable in history.
	 */
	private _computeShunCodeBranchQuery(query: string | undefined, options: IChatAcceptInputOptions | undefined): string | undefined {
		if (options?.shunCodeBranchIntent || query !== undefined || this.getInput().trim()) {
			return undefined;
		}
		if (!this.input.isBranchEligible()) {
			return undefined;
		}
		const requests = this.viewModel?.model.getRequests() ?? [];
		for (let index = requests.length - 1; index >= 0; index -= 1) {
			const text = requests[index].message.text.trim();
			if (text) {
				return text;
			}
		}
	...
	private async _acceptInput(query: { query: string } | undefined, options: IChatAcceptInputOptions = {}): Promise<IChatResponseModel | undefined> {
		...
		const inputValue = options.forceEmptyInput ? '' : (isUserQuery ? this.getInput() : query.query);
