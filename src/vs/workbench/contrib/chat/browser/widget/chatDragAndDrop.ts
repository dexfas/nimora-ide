...
export class ChatDragAndDrop extends Themable {
	...
	private async drop(e: DragEvent): Promise<void> {
		const contexts = this.filterResolvedAttachments(await this.resolveAttachmentsFromDragEvent(e));
	...
	private guessDropType(e: DragEvent): ChatDragAndDropType | undefined {
		...
		if (this.isShunCodeContextOnly()) {
			if (containsDragType(e, TerminalDataTransfers.Terminals)) {
				return ChatDragAndDropType.TERMINAL;
		...
		if (containsDragType(e, CodeDataTransfers.CHAT_REFERENCE)) {
			...
			return ChatDragAndDropType.FILE_EXTERNAL;
		} else if (containsDragType(e, TerminalDataTransfers.Terminals)) {
			return ChatDragAndDropType.TERMINAL;
		} else if (containsDragType(e, CodeDataTransfers.EDITORS)) {
			return ChatDragAndDropType.FILE_INTERNAL;
	...
	private getDropTypeName(type: ChatDragAndDropType): string {
		switch (type) {
			...
			case ChatDragAndDropType.TERMINAL: return localize('terminal', 'Terminal');
	...
	private async resolveAttachmentsFromDragEvent(e: DragEvent): Promise<IChatRequestVariableEntry[]> {
		...
		const shunCodeContextOnly = this.isShunCodeContextOnly();

		if (!shunCodeContextOnly && containsDragType(e, CodeDataTransfers.CHAT_REFERENCE)) {
		...
		if (containsDragType(e, TerminalDataTransfers.Terminals)) {
			...
			try {
				...
				return coalesce(await Promise.all(resources.map(async resource => {
					const terminalContext = this.instantiationService.createInstance(TerminalContext, resource);
					if (!terminalContext.isEnabled(widget)) {
						return undefined;
					}
					return terminalContext.asAttachment(widget);
				})));
			} catch (error) {
				this.logService.warn('Failed to resolve terminal drag context:', error);
				return [];
			}
		}

		if (!shunCodeContextOnly && containsDragType(e, CodeDataTransfers.NOTEBOOK_CELL_OUTPUT)) {
		...
		if (!shunCodeContextOnly && containsDragType(e, CodeDataTransfers.SCM_HISTORY_ITEM)) {
		...
		const markerData = shunCodeContextOnly ? undefined : extractMarkerDropData(e);
		if (markerData?.length) {
			return this.chatAttachmentResolveService.resolveMarkerAttachContext(markerData);
		}

		if (!shunCodeContextOnly && containsDragType(e, CodeDataTransfers.SYMBOLS)) {
		...
		if (editorDragData.length > 0) {
			return coalesce(await Promise.all(editorDragData.map(editorInput => {
				return this.chatAttachmentResolveService.resolveEditorAttachContext(editorInput, shunCodeContextOnly ? { imagesAsFiles: true } : undefined);
			})));
		}
		...
		if (internal) {
			...
			if (uriList.length) {
				return coalesce(await Promise.all(
					uriList.map(uri => this.chatAttachmentResolveService.resolveEditorAttachContext({ resource: URI.parse(uri) }, shunCodeContextOnly ? { imagesAsFiles: true } : undefined))
				));
			}
		}

		if (!shunCodeContextOnly && !containsDragType(e, DataTransfers.INTERNAL_URI_LIST) && containsDragType(e, Mimes.uriList) && ((containsDragType(e, Mimes.html) || containsDragType(e, Mimes.text) /* Text mime needed for safari support */))) {
	...
	private isShunCodeContextOnly(): boolean {
		const widget = this.widgetRef();
		return !!widget && isShunCodeChatWidget(widget);
	}

	private filterResolvedAttachments(attachments: IChatRequestVariableEntry[]): IChatRequestVariableEntry[] {
		if (!this.isShunCodeContextOnly()) {
			return attachments;
		}
		return attachments.filter(attachment => attachment.kind === 'file' || attachment.kind === 'directory' || attachment.kind === 'terminalCommand');
	}
