...
import { IChatBranchRowInfo, IChatBranchService } from '../chatBranchService.js';
...
export class ChatListItemRenderer extends Disposable implements ITreeRenderer<ChatTreeItem, FuzzyScore, IChatListItemTemplate> {
	...
	/**
	 * Renders the branch variant badge (模型 · 分支 i/N · 主线) into the response
	 * footer details line. Called after `renderChatResponseDetails` so the badge
	 * is re-created whenever the details line is refreshed.
	 */
	private renderShunCodeBranchVariantBadge(container: HTMLElement, rowInfo: IChatBranchRowInfo | undefined, isResponse: boolean): void {
		if (!rowInfo || !isResponse || rowInfo.variantCount <= 1) {
			return;
		}
		const parts: string[] = [];
		if (rowInfo.modelId) {
			parts.push(rowInfo.modelId);
		}
		parts.push(rowInfo.variantKind === 'merge'
			? localize('shuncode.branch.mergeBadge', "合并总结")
			: localize('shuncode.branch.variantBadge', "分支 {0}/{1}", rowInfo.variantIndex + 1, rowInfo.variantCount));
		if (rowInfo.isCanonical) {
			parts.push(localize('shuncode.branch.canonicalBadge', "主线"));
		}
		if (container.childElementCount > 0) {
			dom.append(container, $('span.chat-response-details-separator', { 'aria-hidden': 'true' }, '\u2022'));
		}
		dom.append(container, $('span.shuncode-branch-badge', undefined, parts.join(' · ')));
	...
	private renderChatTreeItem(element: ChatTreeItem, index: number, templateData: IChatListItemTemplate): void {
		...
		// ShunCode multi-model: drive per-row footer keys (◀ ▶ / 合并总结 / 采纳 / 删除分支)
		// for the visible variant of each branch group.
		const shunCodeBranchRowInfo = this.viewModel ? this.chatBranchService.getRowInfo(this.viewModel.sessionResource, element.id) : undefined;
		ChatContextKeys.shunCodeBranchVariant.bindTo(templateData.contextKeyService).set(!!shunCodeBranchRowInfo);
		ChatContextKeys.shunCodeBranchCanMerge.bindTo(templateData.contextKeyService).set(shunCodeBranchRowInfo?.canMerge ?? false);
		ChatContextKeys.shunCodeBranchCanAdopt.bindTo(templateData.contextKeyService).set(shunCodeBranchRowInfo?.canAdopt ?? false);
		ChatContextKeys.shunCodeBranchCanDelete.bindTo(templateData.contextKeyService).set(shunCodeBranchRowInfo?.canDelete ?? false);
		ChatContextKeys.shunCodeBranchHasPrev.bindTo(templateData.contextKeyService).set(shunCodeBranchRowInfo?.hasPrev ?? false);
		ChatContextKeys.shunCodeBranchHasNext.bindTo(templateData.contextKeyService).set(shunCodeBranchRowInfo?.hasNext ?? false);
		...
		const updateResponseDetails = () => {
			...
			this.renderShunCodeBranchVariantBadge(templateData.footerDetailsContainer, shunCodeBranchRowInfo, isResponseVM(element));
	...
	private shouldPinPart(part: IChatRendererContent, element?: IChatResponseViewModel): boolean {
		...
		// ShunCode renders Bridge-style tool cards as independent siblings. They must
		// never be absorbed into Chat's generic "Finished with N steps" thinking group.
		if ((part.kind === 'toolInvocation' || part.kind === 'toolInvocationSerialized')
			&& part.toolSpecificData?.kind === 'simpleToolInvocation'
			&& part.toolSpecificData.presentationStyle === 'shuncode') {
			return false;
		}

		// The corresponding native ShunCode invocation remains in the response model
		// to preserve confirmation, cancellation, and terminal lifecycle state. Its
		// completed shell is hidden by the Chat-only renderer, so pinning it would
		// leave an empty "Finished with N steps" container behind.
		if ((part.kind === 'toolInvocation' || part.kind === 'toolInvocationSerialized')
			&& part.toolId.startsWith('shuncode_')
			&& this.configService.getValue<boolean>('shuncode.chat.bridgeStyleUI') !== false) {
			return false;
		}
