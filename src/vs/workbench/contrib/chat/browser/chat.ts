...
export interface IChatAcceptInputOptions {
	...
	/** ShunCode multi-model round intent for this submission. */
	shunCodeBranchIntent?: { kind: 'branch' | 'merge'; groupId?: string };
	/** ShunCode multi-model: send an empty message (used by the merge summary round). */
	forceEmptyInput?: boolean;
	/** ShunCode multi-model: render this submission as a compact labeled row (merge summary round). */
...
export const SHUNCODE_CHAT_PARTICIPANT_ID = 'shuncode.agent';

export function isShunCodeChatWidget(widget: Pick<IChatWidget, 'lockedAgentId' | 'lastSelectedAgent'>): boolean {
	return widget.lockedAgentId === SHUNCODE_CHAT_PARTICIPANT_ID || widget.lastSelectedAgent?.id === SHUNCODE_CHAT_PARTICIPANT_ID;
}
