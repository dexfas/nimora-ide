...
export class AICustomizationManagementEditor extends EditorPane {
	...
	private codexModelsWidget: ChatModelsWidget | undefined;
	...
	private codexContentContainer: HTMLElement | undefined;
	...
	constructor(
		...
		const sectionInfo: Record<string, { label: string; icon: ThemeIcon; description: string }> = {
			...
			[AICustomizationManagementSection.Models]: { label: localize('apiProvider', "API Provider"), icon: Codicon.vm, description: localize('apiProviderDesc', "Set one API endpoint and API key. Available models are discovered automatically.") },
			[AICustomizationManagementSection.Codex]: { label: localize('codex', "Codex"), icon: Codicon.account, description: localize('codexDesc', "Sign in with a ChatGPT subscription and manage which Codex models are available in Chat.") },
			...
			layout: (width, _, height) => {
				...
				if (height !== undefined) {
					...
					if (this.modelsWidget) {
						const modelsWidth = this.modelsWidget.element.clientWidth;
						const modelsHeight = this.modelsWidget.element.clientHeight;
						if (modelsWidth > 0 && modelsHeight > 0) {
							this.modelsWidget.layout(modelsHeight, modelsWidth);
						}
					}
					if (this.codexModelsWidget) {
						const codexWidth = this.codexModelsWidget.element.clientWidth;
						const codexHeight = this.codexModelsWidget.element.clientHeight;
						if (codexWidth > 0 && codexHeight > 0) {
							this.codexModelsWidget.layout(codexHeight, codexWidth);
						}
					}
	...
	private createContent(): void {
		...
		if (hasSections.has(AICustomizationManagementSection.Models)) {
			...
			this.modelsWidget = this.editorDisposables.add(this.instantiationService.createInstance(ChatModelsWidget, { vendor: 'shuncode', presentation: 'api' }));
			this.modelsContentContainer.appendChild(this.modelsWidget.element);

			const modelsFooterElement = DOM.append(this.modelsContentContainer, $('.section-footer'));
			const modelsDescription = DOM.append(modelsFooterElement, $('p.section-footer-description'));
			modelsDescription.textContent = localize('apiProvidersDescription', "The API key is kept in secure secret storage. Models discovered from the configured endpoints become available in the native Chat model picker.");
		}

		// Container for Codex account and models
		if (hasSections.has(AICustomizationManagementSection.Codex)) {
			this.codexContentContainer = DOM.append(contentInner, $('.codex-content-container'));
			const codexBackBar = DOM.append(this.codexContentContainer, $('.section-back-bar'));
			codexBackBar.appendChild(this.createBackArrowButton());
			this.codexModelsWidget = this.editorDisposables.add(this.instantiationService.createInstance(ChatModelsWidget, { vendor: 'shuncode-codex', presentation: 'codex' }));
			this.codexContentContainer.appendChild(this.codexModelsWidget.element);

			const codexFooterElement = DOM.append(this.codexContentContainer, $('.section-footer'));
			const codexDescription = DOM.append(codexFooterElement, $('p.section-footer-description'));
			codexDescription.textContent = localize('codexModelsDescription', "Use the eye control to hide models from the Chat model picker. Thinking effort can be selected per model from the Chat input.");
		}
		...
		if (this.codexModelsWidget) {
			this.editorDisposables.add(this.codexModelsWidget.onDidChangeItemCount(count => {
				this.updateSectionCount(AICustomizationManagementSection.Codex, count);
			}));
			this.codexModelsWidget.fireItemCount();
		}
	...
	private selectSection(section: AICustomizationManagementSection, options?: { showMarketplace?: boolean }): void {
		...
		} else if (section === AICustomizationManagementSection.Codex) {
			this.codexModelsWidget?.focusSearch();
	...
	private updateContentVisibility(): void {
		...
		const isCodexSection = this.selectedSection === AICustomizationManagementSection.Codex;
		...
		if (this.codexContentContainer) {
			this.codexContentContainer.style.display = !isEditorMode && !isMigrationMode && !isDetailMode && isCodexSection ? '' : 'none';
		}
		...
		if (isCodexSection && this.codexModelsWidget) {
			this.codexModelsWidget.render();
			if (this.dimension) {
				this.layout(this.dimension);
			}
		}
	...
	override focus(): void {
		...
		if (this.selectedSection === AICustomizationManagementSection.McpServers) {
			...
			this.modelsWidget?.focusSearch();
		} else if (this.selectedSection === AICustomizationManagementSection.Codex) {
			this.codexModelsWidget?.focusSearch();
