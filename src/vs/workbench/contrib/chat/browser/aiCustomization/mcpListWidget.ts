...
import { IMcpWorkbenchService, IWorkbenchMcpServer, McpConnectionState, McpServerInstallState, IMcpService, IMcpServer, McpToolVisibility } from '../../../../contrib/mcp/common/mcpTypes.js';
...
class McpServerItemRenderer implements IListRenderer<IMcpServerItemEntry | IMcpSessionServerItemEntry | IMcpBuiltinItemEntry, IMcpServerItemTemplateData> {
	...
	renderElement(element: IMcpServerItemEntry | IMcpSessionServerItemEntry | IMcpBuiltinItemEntry, index: number, templateData: IMcpServerItemTemplateData): void {
		...
		// Every configured server has a detail view with connection status and discovered tools,
		// even when the marketplace entry does not provide a description.
		templateData.container.classList.toggle('has-detail', true);
		if (description) {
		...
		if (element.activeSessionServer) {
			...
			templateData.elementDisposables.add(autorun(reader => {
				...
				const toolCount = element.localServer?.tools.read(reader).filter(tool => (tool.visibility & McpToolVisibility.Model) !== 0).length;
				templateData.container.classList.toggle('disabled', disabled);
				this.updateStatus(templateData, element, disabled ? 'disabled' : connectionState?.state, toolCount);
			}));
		}
	}
	...
	private updateKnownServerStatus(templateData: IMcpServerItemTemplateData, element: IMcpServerItemEntry | IMcpBuiltinItemEntry): void {
		templateData.elementDisposables.add(autorun(reader => {
			...
			this.updateStatus(templateData, element, localDisabled ? 'disabled' : activeSessionServer ? (activeSessionServer.enabled ? activeSessionServer.status : 'disabled') : undefined, toolCount);
		}));
	}
	...
	private updateStatus(templateData: IMcpServerItemTemplateData, element: IMcpServerItemEntry | IMcpSessionServerItemEntry | IMcpBuiltinItemEntry, state: McpStatusKind | undefined, toolCount?: number): void {
		templateData.actionDisposables.clear();
		...
		const statusText = toolCount === undefined
			? presentation.label
			: localize('mcpStatusWithToolCount', "{0} · {1} tools", presentation.label, toolCount);
		...
		const showOutput = state === McpServerStatus.Error || state === McpConnectionState.Kind.Error
			...
			: undefined;
		if (showOutput && presentation.icon) {
		...
		if (presentation.icon) {
			statusElement.classList.add(...ThemeIcon.asClassNameArray(presentation.icon));
		}
