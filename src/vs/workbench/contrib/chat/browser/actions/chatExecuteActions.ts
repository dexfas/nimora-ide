...
export class ChatSubmitAction extends SubmitAction {
	...
	constructor() {
		...
		const precondition = ContextKeyExpr.and(
			ContextKeyExpr.or(ChatContextKeys.inputHasSendableContent, ChatContextKeys.inputHasBranch),
...
export class ChatEditingSessionSubmitAction extends SubmitAction {
	...
	constructor() {
		...
		super({
			...
			precondition,
			toggled: {
				condition: ChatContextKeys.inputHasBranch,
				icon: Codicon.arrowUpCompact,
				tooltip: localize('shuncode.branch.send', "Branch send: re-answer the previous question with the current model as a new variant of the same turn."),
			},
			menu: [
				{
					id: MenuId.ChatExecute,
