...
import { IChatWidget, IChatWidgetService, IQuickChatService, isShunCodeChatWidget } from '../chat.js';
import { CHAT_CONTEXT_FILES_AND_FOLDERS_ID, CHAT_CONTEXT_TERMINAL_ID, IChatContextPickerItem, IChatContextPickService, IChatContextValueItem, isChatContextPickerPickItem } from '../attachments/chatContextPickService.js';
...
export class AttachContextAction extends Action2 {
	...
	override async run(accessor: ServicesAccessor, ...args: unknown[]): Promise<void> {
		...
		const shunCodeContextOnly = isShunCodeChatWidget(widget);

		const quickPickItems: IContextPickItemItem[] = [];

		for (const item of contextPickService.items) {
			if (shunCodeContextOnly && item.id !== CHAT_CONTEXT_FILES_AND_FOLDERS_ID && item.id !== CHAT_CONTEXT_TERMINAL_ID) {
				continue;
			}
		...
		instantiationService.invokeFunction(this._show.bind(this), widget, quickPickItems, context?.placeholder, shunCodeContextOnly);
	}

	private _show(accessor: ServicesAccessor, widget: IChatWidget, additionPicks: IContextPickItemItem[] | undefined, placeholder?: string, shunCodeContextOnly = false) {
			...
			handleAccept: async (item: IQuickPickServicePickItem | IContextPickItemItem, isBackgroundAccept: boolean) => {
				...
				if (isIContextPickItemItem(item)) {
					...
					if (!isDone) {
						...
						instantiationService.invokeFunction(this._show.bind(this), widget, additionPicks, placeholder, shunCodeContextOnly);
						return;
					}
		...
		quickInputService.quickAccess.show('', {
			enabledProviderPrefixes: shunCodeContextOnly
			...
			placeholder: placeholder ?? (shunCodeContextOnly
				? localize('chatContext.attach.shuncode.placeholder', 'Select a File, Folder, or Terminal')
	...
	private async _handleQPPick(accessor: ServicesAccessor, widget: IChatWidget, isInBackground: boolean, pick: IQuickPickServicePickItem) {
		...
		if (isIQuickPickItemWithResource(pick) && pick.resource) {
			if (!isShunCodeChatWidget(widget) && /\.(png|jpg|jpeg|bmp|gif|tiff)$/i.test(pick.resource.path)) {
