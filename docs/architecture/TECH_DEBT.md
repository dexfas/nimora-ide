...
| P1 | Bridge public exposure 与业务 presentation 仍耦合 | Phase 7.6 已移除 manager-global todo/progress projection，Work Sessions 直接读 Task；但 generic tool activity、rich diff/terminal cards、tunnel/public endpoint 仍在 Bridge | tool activity timeline 仍把 transport/session presentation 与部分 execution/artifact presentation 混在一起 | 把剩余 execution/artifact presentation 逐步迁到 Task/Artifact generic renderer；Bridge 保留 connection/tunnel/public exposure，不先删 reliability layer |
