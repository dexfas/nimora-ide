...
| P0 | execution ownership 尚未全链路统一 | Phase 5.3–5.4.7 已有 durable owner、opt-in host-requested E2E 与 IDE + Runtime/file provider Router；但 live/default WebMCP 仍由 page Core 执行，session/task grant resolver 也尚未完成 | live 切换时若 approval、恢复或 provider route 漂移仍可能造成副作用重放/越权/任务卡死 | 下一步建立 durable session/task grants，再做登录 provider 的 opt-in release gate；通过前保持默认 observed，不删除 page at-most-once ledger |
