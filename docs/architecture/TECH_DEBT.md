...
| P1 | Gateway 单文件职责过宽 | Phase 6.1–6.4 已拆出 Provider Registry、Integrated Browser、upstream MCP 与 Managed Browser；`server.mjs` 仍集中 Personal Edge long-poll broker 与 WebMCP page-agent session/injection | 剩余 browser-sharing/session state failure isolation 仍有限 | 下一步拆 Personal Edge provider/control broker；WebMCP page host 最后独立，不改变 `/mcp` / `/control/*` compatibility |
