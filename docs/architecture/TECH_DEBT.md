...
| P1 | Gateway 单文件职责过宽 | Phase 6.1 已把 federation ownership/routing 提成 `GatewayProviderRegistry`，Phase 6.2–6.3 已物理拆出 Integrated Browser 与 upstream MCP provider；managed browser、Personal Edge、WebMCP control implementation 仍集中在 `server.mjs` | browser/personal failure isolation 与独立测试边界仍有限 | 继续按 Provider contract 拆 managed/personal provider，再收敛 WebMCP control，不改变 `/mcp` / `/control/*` compatibility |
