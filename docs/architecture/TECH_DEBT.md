...
| P1 | Bridge public exposure 与业务状态仍耦合 | Bridge MCP session 层同时连接 Task shadow、activity/todo、tunnel/public endpoint；其 session reliability 又强于 Gateway lightweight exposure | 直接复用 Gateway adapter 会丢 EventStore/keepalive/capacity/Cloudflare 语义，继续耦合则阻碍 Task 成为 owner | 先做 Phase 7 progress/artifact/UI ownership 迁移，再抽 shared exposure contract；禁止先删 Bridge reliability layer |
