...
| P0 | execution ownership 尚未全链路统一 | Phase 5.3–5.5 已有 durable owner、host-requested E2E、provider Router、durable grants、modal approval/revoke UI 与 application-scoped release gate；默认仍 page-local，用户已登录 provider 的 live roundtrip 尚未完成 | live provider DOM/transport 与真实审批交互仍可能出现 synthetic 测试未覆盖的偏差 | 下一步只做用户已登录 provider 的 opt-in read-only → approval-required live roundtrip；通过前 release gate 默认 false、untrusted workspace 强制 fallback，且不删除 page at-most-once ledger |
...
| P3 | Gateway composition root 仍含 control route wiring | Phase 6.1–6.7 已拆出 Provider Registry、四类 provider、WebMCP Page Host 与 MCP Exposure Adapter；`server.mjs` 主要剩 control routes 与 process bootstrap | route 修改仍集中，但领域/session state 已外移 | 不为 LOC 机械拆 control router；只有多 transport/测试复用有实际收益时再抽 |
| P1 | Bridge public exposure 与业务 presentation 仍耦合 | Phase 7.1 已让 per-session Task strict-own `set_todos/report_progress`，但 manager-global `BridgeManager.todos/activities` 仍作为旧 UI compatibility projection；generic tool activity/tunnel/public endpoint 仍在 Bridge | 旧状态页无法表达多 Task，activity timeline 仍可能把 transport/session presentation 与 Task execution 混在一起 | 建 Task Center/task selection 后把 execution/artifact/progress presentation 改读 Task；Bridge 保留 connection/tunnel/public exposure，不先删 reliability layer |
