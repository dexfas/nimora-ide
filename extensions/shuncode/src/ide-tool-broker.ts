...
const CHAT_CAPTURE_INPUT_KEY = "__shuncodeChatCapture";
...
export class IdeToolBroker implements vscode.Disposable {
  ...
  private async executeTool(name: string, input: Record<string, unknown>): Promise<vscode.LanguageModelToolResult> {
    switch (name) {
      ...
      case "wait": return toolResult(await this.waitImpl(input));
  ...
  /**
   * Blocks for the requested duration. Lets the model wait out a background
   * command or watcher before polling with get_command_output, instead of
   * hammering the terminal in a tight loop.
   */
  private async waitImpl(input: Record<string, unknown>): Promise<string> {
      ...
      this.registrations.push(vscode.lm.registerTool<Record<string, unknown>>(definition.vscodeToolName, {
        prepareInvocation: async (options) => {
          if (definition.name !== "run_command") {
            const title = definition.name === "get_command_output"
              ? "Read command output"
              : definition.name === "send_command_input"
                ? "Send command input"
                : definition.name === "wait"
                  ? "Wait"
                : definition.name;
          ...
          return {
            // Bounded title keeps the collapsed step compact; the full command lives in the
            // confirmation message and the expandable step details.
            invocationMessage: `Run ${summarizeCommand(command)}`,
            confirmationMessages: { title: "Run terminal command", message: markdown },
          };
        },
        invoke: async (options) => this.executeTool(definition.name, asRecord(options.input)),
      }));
    }
  }
