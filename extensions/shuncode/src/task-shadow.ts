...
export class TaskShadowRecorder implements vscode.Disposable, WorkerTaskBindingStore, WorkerExecutionProjectionStore {
  ...
  async recordDiagnosticsArtifact(handle: ShadowExecutionHandle | undefined, args: unknown, resultText: string): Promise<TaskArtifactRef | undefined> {
    if (!handle) return undefined;
    const artifact = taskDiagnosticsArtifact(args, resultText);
    if (!artifact) return undefined;
    return this.safe("record diagnostics artifact", () => this.runtime.recordArtifact(handle.taskId, {
      ...artifact,
      executionId: handle.executionId,
    }));
  }

  async recordDirectoryArtifact(handle: ShadowExecutionHandle | undefined, args: unknown, resultText: string): Promise<TaskArtifactRef | undefined> {
    if (!handle) return undefined;
    const artifact = taskDirectoryArtifact(args, resultText);
    if (!artifact) return undefined;
    return this.safe("record directory artifact", () => this.runtime.recordArtifact(handle.taskId, {
      ...artifact,
      executionId: handle.executionId,
    }));
  }
